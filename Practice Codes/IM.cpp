...
class graph
{
	...
	public:
		graph(long int v);
		bool bfs(long int v,long int w);
	     void addedges(long int a,long int v);
};

graph::graph(long int v)
{
	adj=new list<long int>[v+1]; 	
}
void graph::addedges(long int v,long int w)
{
	adj[v].pb(w);
	adj[w].pb(v);
}
bool graph::bfs(long int a,long int v)
{
	bool visited[500013];
	ms(visited,0);
	queue<long int>q;
