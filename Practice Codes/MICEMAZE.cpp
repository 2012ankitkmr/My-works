...
#define ge(i,L,R) for (long int i = L; i <= R; i++)
...
#define inf   INT_MAX
#define mn  INT_MIN 
long int graph[101][101];
long int floyd[101][101];
long int n,e,k,q,x,y,z;
void floyd_warshall()
{
	...
	ge(s,1,n)ge(i,1,n)ge(j,1,n){
		if(floyd[i][s]+floyd[s][j]<floyd[i][j])
		floyd[i][j]=floyd[i][s]+floyd[s][j];
	}
