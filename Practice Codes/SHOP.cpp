...
{
	ii des;
	des.fi=desx,des.se=desy;
	queue<ii>q;
	q.push(mp(srcx,srcy));
	int posx,posy;
	gl(i,0,27)gl(j,0,27)cost[i][j]=12220;
	cost[srcx][srcy]=0;
	bool visited[30][30];
	ms(visited,0);
	visited[srcx][srcy]=1;
	...
	//	put2n(pa.fi,pa.se);
		gl(i,0,4)
		{
			...
			if(shop[posx][posy]=='X'||posx>=a||posx<0||posy>=b||posy<0)
			continue;
			else
			{
		//printf("%c",shop[posx][posy]);
				if(!visited[posx][posy])
				q.push(mp(posx,posy));
				visited[posx][posy]=1;
				if(shop[posx][posy]=='D'||shop[posx][posy]=='S')
				cost[posx][posy]=min(cost[posx][posy],cost[pa.fi][pa.se]);
				else
				cost[posx][posy]=min(cost[posx][posy],cost[pa.fi][pa.se]+(int)(shop[posx][posy]-'0'));
			}
