...
int dp[101][101][2];
	int w,h;
	char s[101][101];
int compute(int i,int j,int dir)
{

//	if(i==h-1&&j==w-1)
//	{ dir=1;
	
//	return dp[i][j][0]=s[i][j]=='*';
//}
...
if(dir==0)
{
int ret=max(compute(i,j+1,dir)+s[i][j]=='*'?1:0,compute(i+1,j,dir)+s[i][j]=='*'?1:0);
return dp[i][j][dir]=ret;	
}
if(dir==1)
{
int ret=max(compute(i-1,j,dir)+s[i][j]=='*'?1:0,compute(i,j-1,dir)+s[i][j]=='*'?1:0);
return dp[i][j][dir]=ret;	
}
