...
int main()
{
	...
	graph[i].clear();
		...
		gl(i,0,n)
		   if(dis[i]>dis[farthest_point])
		   farthest_point=i;  
	ms(visited,0);
		dfs(farthest_point,0);// this step is done in order to get to the terminal point
		       
		sort(dis,dis+n);// we assumed 0 as any internal node the farthest shall be a terminal node we shall go to that node and find the node which is farthest from the node. that would give the diameter
		int ans=dis[n-1]+dis[0];
		if(ans&1)
		{
		put1n((ans>>1)+1);
	}
