...
int fare[101][101],n,m;
int dp[101][101];
int comput(int i,int city)
{
	if(i<0||i>=m)
	return 0;
	if(city>=n)
	...
	return dp[i][city];
	 ...
	 dp[i][city]=max(max(comput(i-1,city+1)+fare[city][i],comput(i+1,city+1)+fare[city][i]),comput(i,city+1)+fare[city][i]);
	return dp[i][city];
}
...
tests{
	get2(n,m);
	gl(i,0,n)gl(j,0,m)get1(fare[i][j]);
	int ans=0;
	ms(dp,-1);
	gl(i,0,m)
	{ tmp=comput(i,0);
