...
int main() {
	...
	tests
	{
		...
		for(i=0;i<6;i++)
		cin>>colour[i];
		if(colour[0]==colour[2]||colour[0]==colour[3])
		{
			if(colour[0]==colour[4]||colour[0]==colour[5])
			cnt++;
		}
		if(colour[2]==colour[1]||colour[1]==colour[3])
		{
			if(colour[1]==colour[4]||colour[1]==colour[5])
			cnt++;
		}
		if(cnt>0)
		printf("YES\n");
