...
int main()
{
	int n,k,val1=0,val2=0,temp1,left,right,ans=0;
	get2(n,k);
	scanf(" %s",s);
	ms(has,0);
	if(n==1)
	...
	if(k>(n/2))
{
	...
	gl(i,0,n)
	{
		if(s[i]!=s[n-i-1])
		{
			has[i]=abs(s[n-i-1]-s[i]);
			has[n-i-1]=abs(s[n-i-1]-s[i]);
			has[i]=min(has[i],26-has[i]);
			has[n-i-1]=min(has[n-i-1],26-has[n-i-1]);
		}
	}
