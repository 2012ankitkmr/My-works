...
int main()
{
	...
	get1(a[i]);
	res=0;	
	gl(i,0,n)gl(j,0,n)gl(k,0,n)set1.pb(a[i]*a[j]+a[k]);
	gl(i,0,n)gl(j,0,n)gl(k,0,n)if(a[k]==0)continue;set2.pb((a[i]+a[j])*a[k]);
	sort(set1.begin(),set1.end());
	sort(set2.begin(),set2.end());
		
	gl(i,0,set1.size())
	{
		lo=lower_bound(set2.begin(),set2.end(),set1[i])-set2.begin();
		hi=upper_bound(set2.begin(),set2.end(),set1[i])-set2.begin();
		res+=(hi-lo);
	}
	put1n(res);
	return 0;
