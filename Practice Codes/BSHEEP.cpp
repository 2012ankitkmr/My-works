...
#define get2(a,b) scanf("%lf%lf", &a, &b)
...
void convexHull(Point points[],long int n)
{
...
int main()
{
   ...
   //freopen("in.txt","r",stdin);
	...
	{
		sum=sqrt(pow(points[0].x-points[1].x,2)+(pow(points[0].y-points[1].y,2)));
printf("%.2lf\n1 2\n",sum);
	}
	else if(n==3)
	{
		...
		double sum1=sqrt(pow(points[0].x-points[1].x,2)+(pow(points[0].y-points[1].y,2)));
	   double  sum2=sqrt(pow(points[2].x-points[1].x,2)+(pow(points[2].y-points[1].y,2)));
		double sum3=sqrt(pow(points[0].x-points[2].x,2)+(pow(points[0].y-points[2].y,2)));
