...
#define inf   INT_MAX
#define mn  INT_MIN 
int timer[101][101],risk[101][101],maxt,n;
long int nettime,netrisk;
bool mask[101];
void recfloyd(int node,int rsk,int tme)
{
...
ge(i,1,n)
{
	if(i!=node)
	{
		...
		{
			mask[i]=true;
			recfloyd(i,rsk+risk[node][i],tme+timer[node][i]);		
			mask[i]=false;
		}	
