...
class OrderOfOperationDiv2
{
	public:
		int dp[22][21<<1];
		int compute(string[] s,int i,int mask,int m,int n,int size)
		{
			...
			gl(j,0,m)
			{
				if((!mask&(1<<j))&&s[i][j]=='1')
				{
					int temp=min(compute(s,i+1,mask|(1<<j),m,n,size),compute(s,i,mask|(1<<j),m,n,size+1));
