...
{	freopen("in.txt","r",stdin);
	ll n,maxval,temp1,cnt,num;
	double a,b,c,slope,intercept,prevf,prevs;
	int flag;
	tests
	{
		pair<double,double>line;
	multimap<double,double>lines;
	multimap<double,double>::iterator it;
//	pair<multimap<double,double>::iterator,multimap<double,double>::iterator>itp;
	get1(n);	
    maxval=1;
	gl(i,0,n)
	{
		...
		if(b)
		intercept=-c/(double)b;
		else
		intercept=-c/(double)a;
		line.first=slope;
		line.second=intercept;
	
		lines.insert(line);
	
	}
		cnt=1;
		prevf=9999999999999,prevs=9999999999999;	
		set<ii>liness;			
	for(it=lines.begin();it!=lines.end();it++)
		...
		for(it1=liness.begin();it1!=liness.end();it1++)
		{
	//	printf("slope=%lf,intercept=%lf\n\n",(*it1).first,(*it1).second);	
				if((*it1).first==prevf&&(*it1).second==prevs)
			{	
			}
			else if((*it1).first==prevf)
			cnt++;
			else
			cnt=1;
