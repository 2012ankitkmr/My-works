...
long int has[100005];
long int temp[102];
long int dp[50005][100005];
long int compute(long int h,long int i)
{
	if(i>=n||i<0)
	return h;
	if(has[i+1]<=h&&has[i+1]!=0)
	return has[i+1];
	if(dp[h][i]!=-1)
	return dp[h][i];
	long int ret=max(compute(h+1,i+1),max(compute(h-1,i+1),compute(h,i+1)));
	return dp[h][i]=ret;
}
...
int main()
{
	...
	gl(i,0,num)
	{
		has[temp[i]]=temp[num+i];
	}
	//printf("%d ",has[8]);
	
	long int  ans=compute(0,0);
	put1n(ans);
	return 0;
}
