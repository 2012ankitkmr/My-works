...
int binarysearch(int lo,int hi,int val)
{
	if(hi>=lo)
	{
		...
		if(val>=setvalue[mid]&&val<=setvalue[mid+1])
		{
			int flag=0;
			while(val==setvalue[mid])
			{
				mid++;
				flag=1;
			}
		    while(val==setvalue[mid+1])
		    {
		    	mid++;
		    }
		    if(flag)
