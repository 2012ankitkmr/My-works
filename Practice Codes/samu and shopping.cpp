...
int shirt[100005],pant[100005],shoes[100005];
int n;
...
ll compute(int i,int mask)
{
	...
	return dp[i][mask];
	ll ret;
 ret=min(compute(i+1,(mask+1)%3),compute(i+1,(mask+2)%3))+get(mask,i);
...
int main()
{
	tests
	{
		...
		gl(i,0,n)
		{
			get3(shirt[i],pant[i],shoes[i]);
		}
	ll ans= min(compute(0,0),compute(0,1));
	ans=min(ans,compute(0,2));
		printf("%lld\n",ans);
	}
