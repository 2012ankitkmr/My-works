...
int main()
{
	...
	int row[205],col[205],n,k,ans;
	tests
	{
		get2(n,k);
		ge(i,1,n)ge(j,1,n)get1(matrix[i][j]);
		ms(row,0),ms(col,0);
		ge(i,1,n)
		{
			ge(j,1,n)
		...
		ans=154645154;
		ge(i,1,n)if(ans>row[i])ans=row[i];
		ge(i,1,n)if(ans>col[i])ans=col[i];
		put1n(ans);
	}
