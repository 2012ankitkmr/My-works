...
ll cross_mult[78][78],cross_mult_pow_4[78][78];
double compute(ll n, ll q)
{
	...
	gl(i,0,n)
	{
		gl(j,0,n)
		{
			if(i==j)
			continue;
			num=abs(/*a[q]*cross_mult[x[i]][x[j]]+b[q]*cross_mult[y[i]][y[j]]+c[q]*cross_mult[z[i]][z[j]]+*/d[q]);
			den=sqrt(cross_mult_pow_4[x[i]][x[j]]+cross_mult_pow_4[y[i]][y[j]]+cross_mult_pow_4[z[i]][z[j]]);
			ans+=(num/den);
		}
	}
	return ans/(n*(n-1));	
...
int main()
{
	...
	ge(i,1,77)ge(j,1,77){
		cross_mult[i][j]=i-j;
		cross_mult_pow_4[i][j]=pow(cross_mult[i][j],4);
	}
