...
const int IN =0x3f3f3f3f;
	int k,r,m,n,x,y,people[101];
long int dis[21][101];
long int dp[101][20];
int compute(int restaurant,int j)
{
if(j==k||restaurant>=m)
return 0;
...
{
	//put1n(temp);
	if(dis[restaurant][i]<=r)
	return max(people[i]+compute(i+1,j+1),compute(i+1,j));
}	
}
ii loc[21];
int main()
...
gl(j,0,m)	
dis[j][i]=abs(loc[j].fi-x)+abs(loc[j].se-y);	
}	
gl(i,0,m){
gl(j,0,n)
printf("%d ",dis[i][j]); 	
