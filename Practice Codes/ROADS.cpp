...
int totalcoins,cities,roads,source,destination,length,tol;
...
ms(visited,0);
node lcost[106];
gl(i,0,104)lcost[i].len=4544454;	
visited[1]=1;
lcost[1].len=0;
lcost[1].toll=0;
while(!q.empty())
...
{
	if(matrix[p][i].len||matrix[p][i].toll)
	{
		//lcost[i].len=min(lcost[i].len,matrix[p][i].len+lcost[p].len);
	if(lcost[i].len>matrix[p][i].len+lcost[p].len)
	{
		lcost[i].len=matrix[p][i].len+lcost[p].len;
		lcost[i].toll=matrix[p][i].toll+lcost[p].toll;	
	}
		if(!visited[i])
		{
			visited[i]=1;
...
ge(i,1,cities)printf("%d ",lcost[i].len);
pn
ge(i,1,cities)printf("%d ",lcost[i].toll);
pn

if(lcost[cities].toll<=totalcoins)
{
	printf("%d\n",lcost[cities].len);
}	
else
...
freopen("in.txt","r",stdin);
	...
	{
		ms(matrix,0);
		get3(totalcoins,cities,roads);
		gl(i,0,cities)
		{
			get2(source,destination);
			get2(length,tol);
			matrix[source][destination].len=length;
			matrix[source][destination].toll=tol;
		}
		bfs();
	}
