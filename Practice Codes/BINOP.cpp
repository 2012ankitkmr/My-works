...
int main()
{
	int flag,len ,flag2,ans,Azeros,Bzeros,ors,ands,diff;
	tests
	{
		scanf(" %s %s",a,b);
		//checking all zeroes
		...
		else if(Azeros<Bzeros)
		{
			//we perform AND operation
			ans+=(Bzeros-Azeros);
		    ands = ans;
			for(int i = 0;(ands!=0)&&(i<len);i++)
			{
				if(a[i]=='1'&&b[i]=='0')
				{
					a[i]='0';
