...
int matrix[102][102];
int m,n;
void bfs(int srcx,int srcy,int desx,int desy,int time)
{
	queue<ii>q;
	q.push(mp(srcx,srcy));
	int cost[101][101];
	ge(i,1,100)ge(j,1,100)cost[i][j]=12314;
	cost[1][1]=matrix[1][1];
...
else
{
	...
	cost[posx][posy]=min(cost[posx][posy],matrix[posx][posy]+cost[p.fi][p.se]);
	//printf("%d %d %d\n",posx,posy,cost[posx][posy]);	
}	
...
if(posx<=0||posx>m||posy<=0||posy>n)
continue;
cost[desx][desy]=min(cost[desx][desy],cost[posx][posy]+matrix[desx][desy]);
}
if(cost[desx][desy]>time)
printf("NO\n");
else
printf("YES\n%d\n",time-cost[desx][desy]);
}
...
freopen("in.txt","r",stdin);
	...
	{
	get2(m,n);	
	ge(i,1,m)ge(j,1,n)get1(matrix[i][j]);
	get3(desx,desy,t);
