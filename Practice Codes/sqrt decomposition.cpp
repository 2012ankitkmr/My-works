...
#define inf   INT_MAX
#define mn  INT_MIN 
int a[]={45,9,44,10,8,494,11,14,4,554};
int n,sqrtsum[MAXN],st,len,sqrtmin[MAXN];
void incelement(int index,int delta)
{
	a[index]+=delta;
	sqrtsum[index/len]+=delta;	
}
int partsum(int r)
{
	int it=0,res=0;
...
int main()
{
 ...
 len=(int)sqrt(n)+1;
	...
	gl(i,0,n)
	{
		sqrtmin[i/len]=min(sqrtmin[i/len],a[i]);
	}
printA(sqrtmin,0,len-1);
	printf("ans=%d\n",getmin(2,8));
	//printA(sqrtsum,0,len-1);
	//incelement(2,3-a[2]);
	//printA(sqrtsum,0,len-1);	
//	printf("ans=%d\n",getsum(2,6));
	return 0;
