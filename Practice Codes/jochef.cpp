...
struct stackitem*st=(struct stackitem*)malloc(sizeof(struct stackitem)*n);	
ge(i,0,n)
{
while(top>=0&&(i==n||(st[top].heigh)>a[i]))
{
	if(top>0)	
	left=st[top-1].index;
	else left =-1;
	currentarea=(i-left-1)*st[top].heigh;
	--top;
...
{
	int mx,cur_max=0;
	gl(i,1,row)gl(j,0,col)if(field[i][j]==1)field[i][j]=field[i-1][j]+1;
gl(i,0,row)
{
	mx=maxrectanglearea(field[i],col);
	if(mx>cur_max)
	cur_max=mx;	
}
return cur_max;
