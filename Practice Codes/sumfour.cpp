...
#define get4(a,b,c,d) scanf("%ld%ld%ld%ld", &a, &b, &c, &d);
...
long int set1[16000004],set2[16000004];
long int ind,hi,lo,a[4004],b[4004],c[4004],d[4004],len;
...
{//freopen("in.txt","r",stdin);
	...
	get1(n);
	gl(i,0,n)
	get4(a[i],b[i],c[i],d[i]);
	res=0;	
	ind=0;
	gl(i,0,n)gl(j,0,n)set1[ind++]=a[i]+b[j];
	ind=0;
	gl(i,0,n)gl(j,0,n)set2[ind++]=(c[i]+d[j]);
    len=ind;
	sort(set1,set1+len);
	sort(set2,set2+len);
res=0;
    long int i=0,j=len-1;
    ll freq1=1,freq2=1;
    while(i<len && j>=0)
    {
        long long sum=set1[i]+set2[j];
        while(i+1<len && set1[i]==set1[i+1]) freq1++, i++;
        while(j-1>=0 && set2[j]==set2[j-1]) freq2++ , j-- ;
 
        if(sum==0)
            res+=(ll)freq1*(ll)freq2,i++,j--,freq1=1,freq2=1;
        else if(sum>0)
            j-- , freq2=1;        
        else i++, freq1=1;;
    }
