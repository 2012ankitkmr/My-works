...
void find_terminal_node(int n,int m)
{
	int x=0,y=0;
	int flag=0;
	gl(i,0,n){
...
gl(i,0,4)
{
	posx=p.fi+fx[i][0];
	posy=p.se+fx[i][1];
	if(posx>=n||posy>=m||posx<0||posy<0||s[posx][posy]=='#')
	continue;
	else
	{
...
bool visited[505][505];
ms(visited,0);
mystack.push(mp(terminal.fi,terminal.se));
visited[terminal.fi][terminal.se]=1;
queue<ii>q;
q.push(mp(terminal.first,terminal.second));
int posx,posy;
while(!q.empty())
{
	ii p=q.front();
