...
int compute(int index,long int mask)
{
		int temp=curpos(mask);
		if(temp==n)
		return 0;
	...
	gl(i,0,n)
		if(!(mask&(1<<i)))
		{
		int subcost=compute(i,mask|(1<<i));
		int add=c[temp]*abs(a[index]-a[i]);
		 mini=min(mini,add+subcost);
		}
return dp[index][mask]=mini;
}
int main()
{
