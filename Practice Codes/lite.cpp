...
void flip(int node,int st,int en,int i,int j)
{
	if(lazy[node])
	{
		lazy[node]=0;
	...
	flip(2*node,st,(st+en)/2,i,j);
	flip(2*node+1,(st+en)/2+1,en,i,j);
	if(st<en)
tree[node]=tree[node*2]+tree[node*2+1];
}
long int query(int node,int st,int en,int i,int j)
{
	if(st>en||st>j||en<i)
	return 0;
		if(lazy[node])
	...
	return tree[node];
	long int q2=0,q1=0;
	q1=query(node*2,st,(st+en)/2,i,j);
	q2=query(node*2+1,(st+en)/2+1,en,i,j);
	return q1+q2;
}
