...
long int dp[505][10001];
long int compute(int n,int f)
{
	if(f<=0)
	return 0;
	long int m,mi=1111111;
	gl(i,0,n)
	{
	m=value[i]+compute(n,f-weight[i]);
//	put1n(m);
		if(m<=mi)
		mi=m;
	}
...
{int e,f,n;
	...
	f=f-e;
	get1(n);
	gl(i,0,n)get2(value[i],weight[i]);
	//ms(dp,-1);
	long int ans=compute(n,f);
printf("%ld\n",ans);
	}
	return 0;
}
