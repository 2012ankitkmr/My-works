...
int dp;
int maxb[22][22];
int compute(int x,int y,int n,int m,int amount)
{
	if(x>n||y>m)
	return amount;
if(maxb[x][y]!=-1)
...
if(maxb[x][y]<amount&&x==n&&y==m)
maxb[x][y]=amount;
	dp=max(compute(x+1,y,n,m,amount-po[x+1][y]),compute(x,y+1,n,m,amount-po[x][y+1]));
