...
#define inf   INT_MAX
#define mn  INT_MIN 
ll arr[100005],tree[300006];
ll getmid(ll a,ll b)
{
return a+(b-a)/2;
}
ll query(ll ss,ll se,ll a,ll b,ll node)
{
if(ss>b||se<a||a>b||ss<0||ss>se||a<0)
return 0;
if(ss>=a&&se<=b)
return tree[node];
ll mid=getmid(ss,se);
return query(ss,mid,a,b,2*node+1)+query(mid+1,se,a,b,2*node+2);
}



ll construct_util(ll ss,ll se,ll node)
{
if(ss==se)
{
tree[node]=arr[se];
...
ll mid=getmid(ss,se);
tree[node]=construct_util(ss,mid,2*node+1)+construct_util(mid+1,se,2*node+2);
return tree[node];
}
...
{
ll temp=construct_util(0,n-1,0);
}
void swap(ll&a,ll&b)
{
ll temp;
