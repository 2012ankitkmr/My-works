...
#define mn  INT_MIN 
bool matrix[101][101];
int indegree[101],min_degree=111;
void indegree_check(int n)
{
	ge(i,1,n)ge(j,1,n)
	{
		if(matrix[i][j])
...
//	printA(indegree,1,n);
///	ge(i,1,n)if(indegree[i]<min_degree)min_degree=indegree[i];
}
int topological[101],topological2[101];
void topological_order(int n)
{
	...
	printA(topological,1,n);
	for(int i=1;i<=n;i++)
	{	topological2[topological[i]]=i;
	}
	printA(topological2,1,n);
}
