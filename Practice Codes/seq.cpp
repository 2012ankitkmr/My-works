...
void mult(matrix &m,matrix &m1,matrix &m2)
{
	ms(m.X,0);
	long int i,j,k;
	gl(i,0,size)gl(j,0,size)gl(k,0,size)m.X[i][k]=(m.X[i][k]=m1.X[i][j]*m2.X[j][k])%MOD;
}
...
int main()
{
	long int t,b[10],c[10],i,j;
	int n;
	matrix m0,ret;
	tests{
		get1(size);
		for(i=0;i<size;i++)get1(b[i]);
		for(j=0;j<size;j++)get1(c[i]);
		get1(n);
		n--;
		if(size>n)
		{
		...
		{
			ms(m0.X,0);
			gl(i,0,size)m0.X[0][i]=c[i];
			gl(i,1,size)m0.X[i][i-1]=1;
			
			ret=pow(m0,n-size+1);
			ll ans=0;
			gl(i,0,size)ans=(ans+ret.X[0][i]*b[size-i-1])%MOD;
			put1n(ans);
		}
