...
int num[MAX], dp[MAX][1024];
 
int nume(char* s, char c)
{
	int temp = 0;
	for(int i = 9; i >= 0; i--) {
		if(s[i] == c) 
...
int cnt[1026],numa[1029];
int P[100005];
char s[12];
int main()
{
	freopen("in.txt","r",stdin);
	int  n, lim = 1024, A,new_n,prev;
...
prev=99999999;
 ...
 //put1n(cnt[numa[0]]-1);
		...
		//	printf("here\n");
			gl(j,0,lim) {
				dp[i][j] = (dp[i - 1][j] + dp[i - 1][j ^ numa[i]]) % MOD;
		    
		     	//dp[i][j]=(dp[i][j]*P[cnt[numa[i]]-1])%MOD;
			           }
