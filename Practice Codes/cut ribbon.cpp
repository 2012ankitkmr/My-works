...
int n,a[4];
int dp[8005][4005];
int compute(int amount,int cnt)
{
	if(amount==n)
	{
		//printf("returning cnt=%d from amount %d \n",cnt,amount);
		return cnt;	
	}
	if(amount>n)
	{
...
//gl(i,0,3)
{
tot=max(compute(amount+a[0],cnt+1),max(compute(amount+a[1],cnt+1),compute(amount+a[2],cnt+1)));	
//printf("total=%d\n",tot);
}
	return dp[amount][cnt]=tot;
}
